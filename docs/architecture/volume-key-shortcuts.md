...
| 槽位 | prefs key | 出厂默认 |
| --- | --- | --- |
| 长按音量上键（阈值可调，默认 500ms） | `volume_gesture_long_press_up` | `none` |
| 长按音量下键（阈值可调，默认 500ms） | `volume_gesture_long_press_down` | `quick_record` |
| 双击音量上键（300ms 窗口） | `volume_gesture_double_click_up` | `none` |
| 双击音量下键（300ms 窗口） | `volume_gesture_double_click_down` | `quick_text_note` |

> 长按触发阈值见下方「长按触发阈值（可调档位）」；双击窗口仍为固定 300ms 硬编码。
...
### 长按触发阈值（可调档位，2026-09-19）

两个「长按」槽位共用一个触发阈值，设置页「音量键快捷操作」ChoiceChip 选档（快 400 / 标准 500 / 慢 800 / 很慢 1200ms），默认 500ms（历史硬编码值）。

- **prefs key**：`volume_long_press_ms`（⚠️ 落盘类型 **Long**——Flutter `setInt` 在 Android 端即走 `putLong`（Dart int 64 位），Kotlin 必须 `getLong` 读，`getInt` 会 ClassCastException 崩服务进程，2026-09-19 真机炸过；同 `pro_trial_deadline_ms` 先例；读取加 `flutter.` 前缀）。Dart 写入方为二级页 `volume_key_settings_page.dart` 的 `_saveLongPressMs`（唯一写入方），Kotlin `getLongPressDurationMs()` 每次 `ACTION_DOWN` 实时读落盘——与手势槽位动作同模式，无 MethodChannel、App 未打开也生效
- **档位校验双侧同规则**：缺失/脏值回落 500。档位集合是跨端硬编码副本（Dart `VolumeLongPressMs.choices` ↔ Kotlin `LONG_PRESS_MS_CHOICES`），改档位必须双侧同步
- **下限 400ms 是刻意设计**：刻意单击音量键的按压时长约 100~300ms，阈值低于它会把按得偏重的单击误判成长按（误触录音/悬浮窗，且长按在 DOWN 后到点即触发、松手无法反悔）。上限 1200ms 再长手感明显发钝
- **与单击/双击无结构性冲突**：长按计的是 DOWN 持续时长、双击计的是两次 UP 间隔、单击是双击窗口超时兜底，三者时钟互相独立；`wasLongPress` 短路保证长按后的 UP 不进双击序列，任何档位下均成立
...
- **VolumeKeyAccessibilityService.kt** - 无障碍服务，统一手势状态机（`onKeyEvent`）
  ...
  - **ACTION_DOWN**：取消 pendingSingleClick；长按槽有动作才缓存 `currentLongPressAction` + 启动长按计时（`longPressHandler.postDelayed`，时长读 prefs 档位，默认 500ms）。长按槽=none 时不启动计时——按住不放无动作，抬起走调音量路径
...
- `volume_long_press_ms` (Long) - 长按触发阈值毫秒（Flutter `setInt` 写入 `_saveLongPressMs` 但落盘即 Long，原生必须 `getLong` 读否则崩，见上文；缺失/脏值回落 500）
...
1. **录音中长按仍触发**——`longPressRunnable`（阈值档位时长，默认 500ms）不被 `is_recording` 拦截，`quick_record` / `overlay_record` 的 toggle 停录语义依赖于此（长按一下开始、再长按一下停止）
