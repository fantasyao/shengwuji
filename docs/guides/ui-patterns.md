...
- 冷启动时显示，完成初始化后自动消失
- **视觉设计**（2026-09-24 重设计，方案 B「深海极光」，用户从配色预览中选定）：
  - 默认主题：深海三段渐变背景（`#0D1B2E→#13253C→#16304A`，`splashGradient`）+ 图标区冷蓝辉光（`#5A5E9EDC` 径向淡出，`splashGlow`）
  - 其余主题/自定义主题：回落各自纯色 `splashBackground`（SplashShell 统一处理两种路径）
  - 藏青版应用图标（120x120，`assets/icon/icon2.png` 与桌面图标同为 `#2C3E50` 底白线稿；2026-09-24 前是薄荷绿渐变底，与藏青背景撞色）
  - 应用名称 + "使用离线语音模型在本地完成识别，不上传任何数据"标语
  - 白底授权按钮（文字色随 `splashBackground`）
- **进度条（两段式，引擎延迟加载后无"加载引擎"阶段）**：
  - 0% → "正在准备语音模型..."（模型路径预加载）
  - 50% → 未授权时停在此处，页面显示"授权并开始"按钮等待用户点击；点击后 → "请求麦克风权限..."
  - 100% → 显示主界面（引擎不在此加载，首次录音时由日记页/随手记页触发，见 [postmortem-lazy-model-loading.md](postmortem-lazy-model-loading.md)）
- Android 12+ 系统启动画面背景色为 `#0D1B2E`（`values-v31/styles.xml` 的 `windowSplashScreenBackground`），与 Flutter 深海渐变衔接不跳色
...
- 电脑访问（局域网日记服务，按拍板保留一级页面）
- **支持作者分区**：Pro 付费解锁入口（诊断区与关于区之间）
- 识别与修正 → 二级页：热词编辑（已并入全量备份/恢复）+ 修正对管理入口 + 同音词上下文纠错开关
- AI 应用分享 → 二级页：日记分享跳转应用单选
- 音量键快捷操作 → 二级页：无障碍服务三态状态 + 4 手势槽位配置 + 录音静音/说完自动停止开关（主页入口行保留无障碍状态点，resume 自动重查）
- 悬浮窗 → 二级页：自动隐藏时长/停靠侧/贴边竖线两开关（Pro 门禁走 `ProGate.tryAccess`）
- 关于 → 二级页：17 版更新日志时间线 + 导出运行日志 + 开源许可
